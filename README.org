...
Two backends are registered: *OpenRouter* for hosted models (the
default) and *llama.cpp* for the local one on =localhost:8999=.
OpenRouter is a single API — and a single key — in front of every
hosted model worth using, and it speaks the OpenAI protocol, so
=gptel-make-openai= is the correct constructor for it.
...
The key is never stored in this repo. =my-gptel-openrouter-key= looks
...
1. =~/.authinfo.gpg= — the preferred spot, same file Gnus already uses:
   ...
   #+end_src
2. the =OPENROUTER_API_KEY= environment variable, for keys injected by
   home-manager or sops;
3. a =read-passwd= prompt, cached for the session, as a last resort.
...
from =https://openrouter.ai/api/v1/models= and replaces the backend's
list in place, so the model picker in =C-c g m= shows what actually
exists today. Nothing else needs restarting.
...
=local= preset (below) is the quick way back to llama.cpp — worth
using for anything where a 9B model is good enough, since every
OpenRouter request costs money.
