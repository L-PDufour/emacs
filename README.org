...
Two different Capf setups, on purpose:

- Non-Eglot buffers (prog/text/org) get =tempel-complete= prepended to
=completion-at-point-functions=, which lists templates in the popup so
they stay discoverable.
- Eglot-managed buffers get a single =cape-capf-super= merging
=eglot-completion-at-point=, =tempel-complete= and =cape-file= (see the
Cape section). A sequential list cannot work there: completion stops at
the first non-nil Capf, and Eglot always answers. Supering folds the
templates into the same popup as the LSP candidates, prefix-filtered so
they only appear when their name matches.

=M-+= (=tempel-complete=) and =M-*= (=tempel-insert=) still browse
templates on demand anywhere, including Eglot buffers.
