...
=my/default-completion-ui= decides which completion front-end the rest of
this file builds. =nil= (the default) is the setup we have always run:
Vertico for the minibuffer, Corfu for in-buffer completion. Non-nil
leaves both disabled and hands the job to the built-in =*Completions*=
buffer, which grew eager display and eager update in Emacs 31 — see the
[[*Default Completions (Emacs 31)][Default Completions]] section for what that turns on.

Flip it here or with =M-x customize-variable RET my/default-completion-ui=,
then restart. It is defined this early because [[*Window Display Rules][Window Display Rules]]
...
When =my/default-completion-ui= is on, =*Completions*= is dropped from the
side-window rule. A bottom side window pinned to =0.25= of the frame
overrides =completions-max-height= and never opens under the window you
are completing in, which is most of what makes in-buffer =TAB= completion
pleasant. Left to itself, =*Completions*= splits the selected window and
shrinks to fit.
...
installed, so the built-in UI gets a clean field.
...
Also skipped when =my/default-completion-ui= is on. =tab-always-indent= is
already =complete= in [[*Defaults][Defaults]], so with Corfu out of the way in-buffer
=TAB= falls through to the same =*Completions*= buffer the minibuffer uses.
Cape stays either way: it only feeds =completion-at-point-functions=.
...
Everything below is inert unless =my/default-completion-ui= is non-nil.
It follows two write-ups on the built-in interface: Protesilaos'
...
[[https://www.rahuljuliato.com/posts/completions-buffer-is-now-enough][The Completions Buffer Is Now Enough]].

The two settings that carry the whole thing are new in Emacs 31:
...
and almost nothing asks. Set to =t= they turn =*Completions*= into a live
narrowing list you never summon with =TAB=. They are guarded with
=boundp= so this section stays harmless on Emacs 30 and older, where you
get a static, =TAB=-driven =*Completions*= instead.

=minibuffer-visible-completions= is the other half: point stays in the
minibuffer while =C-n= / =C-p= walk the visible list and =RET= takes the
highlighted candidate. ='up-down= rather than =t= leaves left and right
moving through your input.
...
work without a second reach. Prot prefers =nil= — with eager display you
rarely press =TAB= anyway and never lose the minibuffer. Try both.

=my/completion-category= is Prot's helper for writing
=completion-category-overrides=. It is defined under either UI: focus a
minibuffer prompt, =M-:=, evaluate =(my/completion-category)=, and the echo
area gives you the category symbol to write an override for. It needs
