...
* Vanilla Plus — Enhanced Emacs (No Modal Editing)
:PROPERTIES:
:header-args:emacs-lisp: :tangle init.el
:END:

Instead of modal editing, we enhance standard Emacs with:
- *Repeat maps* — press =C-M-f= once, then tap =f f f= (no more chording)
- *Unified jump navigation* — =M-.= to jump, =M-,= to go back (xref + mark ring)
- *Smart C-a* — first press goes to indentation, second to column 0
- *Line operations* — duplicate, move up/down, smart commenting
- *Auto-centering* — cursor re-centers after jumps (like vim's =zz=)
...
** Unified Jump Navigation (C-o / C-i)

Jump back through xref history AND mark ring with a single key.
Like vim's =C-o= / =C-i= but for vanilla Emacs.
...
Press =C-M-f= once, then just tap =f=, =b=, =u=, =d=, etc. to keep
navigating. No more holding three modifier keys.
...
=C-x o= then tap =o o o= to cycle windows, or =h j k l= to navigate.
...
=C-x <right>= then tap =<right> <right>= to cycle buffers.
...
=M-.= to jump, =M-,= to go back, then =, , ,= to keep going back.
...
incrementally expand selection by semantic units using tree-sitter.
Press =C-== to expand, =C--= to contract.
...
Toggle project-scoped popups — shells, compilation, diagnostics.
Grouped by project via tabspaces so each workspace has its own set.
...
The =C-c= prefix keymaps give structured access to common commands.
All bindings work directly — no modal editing needed.

| Keys          | Command               |
|---------------+-----------------------|
| =C-c f a=     | project-find-file     |
| =C-c f b=     | project-find-buffer   |
| =C-c f r=     | consult-recent-file   |
| =C-c f s=     | save-buffer           |
| =C-c w v=     | split-window-right    |
| =C-c w s=     | split-window-below    |
| =C-c w h/j/k/l= | windmove           |
...
These keymaps are accessed directly via =C-c f a=, =C-c w v=, etc.
...
Tell which-key what each =C-c= prefix does so you get helpful
hints when pressing =C-c= and waiting.
