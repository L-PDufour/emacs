...
#+begin_src emacs-lisp
  ...
  ;; Defer font-lock until input stops — eliminates jit-lock antiblink overhead
...
#+begin_src emacs-lisp
  (use-package emacs
    ...
    ;; hl-line only where useful — global mode redraws on every cursor move
...
#+begin_src emacs-lisp
  ;; Emacs 30 injects ispell-completion-at-point in text modes; disable it
  ;; because it errors without a system word list (common in Nix envs)
...
#+begin_src emacs-lisp
  (use-package doom-modeline
    ...
    ;; Disable the segments that are expensive on every redisplay
    (doom-modeline-github nil)           ; no GitHub API polling
    (doom-modeline-env-version nil)      ; no language version subprocess
    (doom-modeline-minor-modes nil)
    (doom-modeline-enable-word-count nil)
    (doom-modeline-time nil)
    ;; VCS: truncate long branch names; the segment itself still runs
    ;; but shorter strings redisplay faster
    (doom-modeline-vcs-max-length 15)
    ;; Simpler flymake indicator — avoids per-redisplay diagnostic scan
    (doom-modeline-check-simple-format t)
    (doom-modeline-buffer-file-name-style 'truncate-upto-project)
    (doom-modeline-workspace-name t))

  ;; Stop VC from polling file state on every auto-revert tick —
  ;; this is what makes doom-modeline-segment--vcs expensive in large repos
...
#+begin_src emacs-lisp
  (use-package indent-bars
    ...
    ;; Current-depth highlight tracks the cursor on every move — skip it
...
| Normal | =I / A=           | insert at first non-blank / append EOL   |
| Normal | =o / O=           | open line below / open line above        |
| Normal | =x / X=           | delete char forward / select line        |
| Normal | =d / s=           | kill (cut) into kill-ring / delete       |
| Normal | =c=               | change (delete + insert)                 |
| Normal | =y / p=           | yank (copy) / paste                      |
| Normal | =u / U=           | undo / undo in selection                 |
| Normal | =gg / G=          | buffer start / buffer end                |
...
Normal mode follows meow's default QWERTY scheme with a handful of
overrides for vim muscle memory: =x= deletes a char forward (not
line-select), =o= / =O= open a line below/above and enter insert (vim
=o=/=O=), =A= / =I= append at end-of-line / insert at first
non-blank (vim =A=/=I=), and =gg= / =G= jump to buffer start/end (vim
=gg=/=G=). =d= kills into the kill-ring like vim's register-delete;
the previous non-killing delete moved to =s=. Whole-line operations
(vim's =dd=/=yy=/=cc=) now read as =X d= / =X y= / =X c= since
line-select moved to =X=.

#+begin_src emacs-lisp
  (defun meow-vim-append-eol ()
    "Vim-style `A': move to end of line, then enter insert state."
    (interactive)
    (end-of-line)
    (meow-insert))

  (defun meow-vim-insert-bol ()
    "Vim-style `I': move to first non-blank, then enter insert state."
    (interactive)
    (back-to-indentation)
    (meow-insert))

  (defvar my-meow-g-map
    (let ((map (make-sparse-keymap)))
      (define-key map "g" #'beginning-of-buffer)  ; gg
      (define-key map "c" #'meow-cancel-selection)
      (define-key map "r" #'meow-grab)
      (define-key map "l" #'meow-goto-line)
      map)
    "Vim-style g-prefix map for meow normal state.")
  ...
  (use-package meow
    ...
    (defun meow-setup ()
      ...
      (meow-normal-define-key
       ...
       '("]" . meow-end-of-thing)
       '("a" . meow-append)
       '("A" . meow-vim-append-eol)     ; vim A: end-of-line then insert
       '("b" . meow-back-word)
       '("B" . meow-back-symbol)
       '("c" . meow-change)
       '("d" . meow-kill)               ; vim d: kill into kill-ring; with region, removes it
       '("D" . meow-backward-delete)
       '("e" . meow-next-word)
       '("E" . meow-next-symbol)
       '("f" . meow-find)
       '("g" . my-meow-g-map)           ; g-prefix: gg=buf-start, gc=cancel, gr=grab, gl=goto-line
       '("G" . end-of-buffer)           ; vim G: jump to buffer end
       '("h" . meow-left)
       '("H" . meow-left-expand)
       '("i" . meow-insert)
       '("I" . meow-vim-insert-bol)     ; vim I: first-non-blank then insert
       '("j" . meow-next)
       '("J" . meow-next-expand)
       '("k" . meow-prev)
       '("K" . meow-prev-expand)
       ...
       '("n" . meow-search)
       '("N" . meow-pop-search)
       '("o" . meow-open-below)         ; vim o: open line below
       '("O" . meow-open-above)         ; vim O: open line above
       '("p" . meow-yank)
       '("P" . meow-yank-pop)
       '("q" . meow-quit)
       '("Q" . meow-goto-line)
       '("r" . meow-replace)
       '("R" . meow-swap-grab)
       '("s" . meow-delete)             ; non-killing delete (swapped with d)
       '("t" . meow-till)
       '("u" . meow-undo)
       ...
       '("w" . meow-mark-word)
       '("W" . meow-mark-symbol)
       '("x" . delete-char)             ; vim x: delete char forward
       '("X" . meow-line)               ; line-select (for X d/y/c whole-line ops)
       '("y" . meow-save)
       '("Y" . meow-sync-grab)
...
| =SPC s l=         | =consult-line=                  | via search-map      |
| =SPC s G=         | =consult-ripgrep=               | project-wide        |
...
** Meow Leader Namespaces

Wired here — after all keymaps are defined — so meow can reference
them without forward-reference errors.  Each namespace letter is bound
to a proper keymap; meow never sees multi-key sequences in
=meow-leader-define-key=, which avoids the "non-prefix key" conflict
with meow's default single-key forwarding.

#+begin_src emacs-lisp
  ;;; Additional keymaps for leader namespaces not covered by C-c maps
  (defvar my-buffer-keymap    (make-sparse-keymap))
  (defvar my-git-keymap       (make-sparse-keymap))
  (defvar my-help-keymap      (make-sparse-keymap))
  (defvar my-open-keymap      (make-sparse-keymap))
  (defvar my-toggle-keymap    (make-sparse-keymap))
  (defvar my-error-keymap     (make-sparse-keymap))
  (defvar my-ai-keymap        (make-sparse-keymap))
  (defvar my-workspace-keymap (make-sparse-keymap))

  ;;; b = buffers
  (define-key my-buffer-keymap "b" #'consult-buffer)
  (define-key my-buffer-keymap "d" #'kill-current-buffer)
  (define-key my-buffer-keymap "n" #'next-buffer)
  (define-key my-buffer-keymap "p" #'previous-buffer)
  (define-key my-buffer-keymap "s" #'save-buffer)
  (define-key my-buffer-keymap "i" #'ibuffer)

  ;;; g = git
  (define-key my-git-keymap "g" #'magit-status)
  (define-key my-git-keymap "b" #'magit-blame)
  (define-key my-git-keymap "l" #'magit-log-current)
  (define-key my-git-keymap "f" #'magit-log-buffer-file)
  (define-key my-git-keymap "d" #'magit-diff-buffer-file)

  ;;; h = help
  (define-key my-help-keymap "f" #'describe-function)
  (define-key my-help-keymap "v" #'describe-variable)
  (define-key my-help-keymap "k" #'describe-key)
  (define-key my-help-keymap "m" #'describe-mode)
  (define-key my-help-keymap "p" #'describe-package)
  (define-key my-help-keymap "." #'display-local-help)

  ;;; o = open
  (define-key my-open-keymap "t" #'my-project-eat)
  (define-key my-open-keymap "e" #'eshell)
  (define-key my-open-keymap "m" #'ghostel)
  (define-key my-open-keymap "-" #'dired-jump)
  (define-key my-open-keymap "p" #'popper-toggle)

  ;;; t = toggles
  (define-key my-toggle-keymap "l" #'display-line-numbers-mode)
  (define-key my-toggle-keymap "w" #'visual-line-mode)
  (define-key my-toggle-keymap "h" #'hl-line-mode)
  (define-key my-toggle-keymap "f" #'flymake-mode)
  (define-key my-toggle-keymap "d" #'diff-hl-mode)

  ;;; e = errors
  (define-key my-error-keymap "e" #'consult-flymake)
  (define-key my-error-keymap "n" #'flymake-goto-next-error)
  (define-key my-error-keymap "p" #'flymake-goto-prev-error)
  (define-key my-error-keymap "l" #'flymake-show-buffer-diagnostics)

  ;;; a = AI / gptel
  (define-key my-ai-keymap "a" #'gptel)
  (define-key my-ai-keymap "s" #'gptel-send)
  (define-key my-ai-keymap "m" #'gptel-menu)
  (define-key my-ai-keymap "f" #'gptel-add-file)

  ;;; TAB = workspaces
  (define-key my-workspace-keymap (kbd "TAB") #'tabspaces-switch-or-create-workspace)
  (define-key my-workspace-keymap "b" #'tabspaces-switch-to-buffer)
  (define-key my-workspace-keymap "d" #'tabspaces-close-workspace)
  (define-key my-workspace-keymap "k" #'tabspaces-kill-buffers-close-workspace)

  ;;; Also add code keys not in my-code-keymap yet
  (define-key my-code-keymap (kbd "D") #'xref-find-definitions-other-window)
  (define-key my-code-keymap (kbd "f") #'eglot-format)

  ;;; Wire all namespaces into the meow leader (single-key → keymap, no sequences)
