...
The Capfs use =tempel-expand=, which fires only on an exact template
name. =tempel-complete= is unsuitable in a shared Capf list: when the
text at point matches no template, =tempel--prefix-bounds= falls back to
=bounds-of-thing-at-point=, so it offers *every* template at nearly any
symbol and hides the LSP candidates behind it. Browse templates on
purpose with =M-+= (=tempel-complete=) or =M-*= (=tempel-insert=).

Note there is no =tempel-trigger-prefix= any more — upstream removed it
in favour of the prefix heuristic above, so setting it did nothing.
