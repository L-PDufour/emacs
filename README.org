...
=test-minibuffer.el= is not tangled from here. It is a standalone
=emacs -Q= playground for the built-in =*Completions*= UI; see
[[*Default Completions (Emacs 31)][Default Completions]].
...
[[file:test-minibuffer.el][test-minibuffer.el]] is the same interface with none of this config
around it, straight from Rahul's post: =emacs -Q --load test-minibuffer.el=.
It uses stock =flex= plus his =flex-noinsert= style instead of Orderless,
so it is also where you can see the =TAB=-inserts-a-wrong-candidate
behaviour that Orderless spares us.
