...
=consult-preview-at-point-mode= is defined in =consult.el=, so its
=:hook= lives on the =consult= block above, not here — putting it on
=embark-consult= makes =use-package= generate an autoload that
(wrongly) points at =embark-consult.elc=, which doesn't define that
function and throws a =run-hooks= autoloading error the first time
an Embark collect buffer opens.
...
Evil brings real Vim emulation — operator + motion grammar (=dw=,
=ciw=, =yy=, =p=, =.=...) instead of held-down chords, so common
edits are two bare keystrokes with no =Ctrl=/=Meta=. =SPC= opens a
Doom-style leader keypad (via the vendored =evil-keypad=) that
replays the existing =C-c= maps defined in the Keybindings section
below, so nothing is bound twice.

Normal state is scoped to =prog-mode= buffers only (see below) —
everywhere else, and while actually typing, this behaves like plain
Emacs.
...
| Normal | =i a I A=         | insert before/after, line start/end   |
| Normal | =o O=             | open line below / above               |
| Normal | =v V C-v=         | char / line / block visual selection  |
| Normal | =d c y p=         | delete / change / yank / put          |
| Normal | =u C-r=           | undo / redo                           |
| Normal | =/ ?=             | search forward / backward             |
| Normal | =gcc / gc=        | comment line / comment operator       |
| Normal | =gd / gr / gb=    | goto def / refs / jump back           |
| Normal | =:=               | ex command                            |
| Normal | =SPC=             | keypad leader (see Keybindings)       |
| Insert | =C-g= / =<ESC>=   | back to normal state                  |
...
** Evil Scope: Programming Buffers Only

Vim motions/operators only earn their keep where you're editing
structured code — everywhere else (Dired, Magit, Org, prose,
=eshell=, the minibuffer, ...) this uses plain Emacs state, i.e.
ordinary Emacs bindings with no modal layer at all. That covers
"insert mode" too: Emacs state has no normal/insert split, you can
just type.

=evil-default-state= above sets the fallback; this hook promotes any
=prog-mode=-derived buffer to Evil normal state and is intentionally
appended (depth 90) so it runs *after* Evil's own
=after-change-major-mode-hook= entry and after whatever
=evil-collection= wired up for that major mode — otherwise those
would win and put non-programming buffers back into normal state.
...
Extends Evil bindings into Dired, Magit, Vertico, the minibuffer,
Org, and the rest of the modes this config uses — the same coverage
Doom gets from its =:editor evil= module.
...
** Evil Surround and Commentary

=ys=/=cs=/=ds= for surround (like =vim-surround=), =gcc=/=gc= for
comments (like =vim-commentary=).
...
** Evil Keypad (SPC leader)

Vendored in =site-lisp/evil-keypad.el= (not on any package archive).
=SPC c f a= is translated straight into =C-c f a=, =SPC x= into
=C-x=, =SPC m x= into =M-x=, etc. — a Doom-style leader without
pulling in =general.el= or redeclaring every binding a second time.

=evil-keypad='s default trigger states include Evil's =emacs= state,
which would steal the literal =SPC= keypress (no more typing spaces)
in every buffer this config now puts in =emacs= state. Restrict it
to =normal=/=visual=/=motion=, none of which self-insert on =SPC=.
...
Evil normal-mode commands with a =repeat-map= registered (e.g.
=flymake-goto-next-error=, =windmove=, =xref=) still repeat via
Emacs's built-in =repeat-mode=, and =.= repeats the last change
command the Vim way. No extra configuration is needed — the
=repeat-mode= and all repeat maps defined earlier in this file work
transparently once Evil is loaded.
