...
Flycheck is installed but *not* enabled — Flymake above is the active
checker.  There is no hook and no =global-flycheck-mode=; nothing turns it
on by itself.  It is here so third-party packages that expect Flycheck can
load it, and so =M-x flycheck-mode= works in a single buffer when a
Flycheck-only checker is worth reaching for.

Running both checkers in one buffer means two sets of overlays and two sets
of subprocesses, so switching Flycheck on locally switches Flymake off in
that buffer.  Turning Flycheck back off leaves Flymake off too; =M-x
flymake-mode= restores it.
...
Offline API documentation from [[https://devdocs.io][devdocs.io]].  Docsets are downloaded on
demand with =M-x devdocs-install= and stored under =var/devdocs/=, which
=.gitignore= already covers.  Nothing is fetched at startup.

=devdocs-current-docs= is buffer-local and decides which docsets
=devdocs-lookup= searches without a prefix argument.  A single
=prog-mode-hook= sets it from =my-devdocs-major-mode-docs=; modes missing
from that alist just fall back to prompting.  The slugs are the ones
DevDocs itself uses, so versioned entries (=python~3.12=) need to match the
docset actually installed.

Bound to =C-h D= globally and to =C-c l k= in the code map.  =C-u C-h D=
picks docsets for the current buffer interactively.
