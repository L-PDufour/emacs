...
(defconst my/sync-root "/var/lib/syncthing/Sync/"
  "Root of the Syncthing shares.")

(defconst my/org-root (expand-file-name "personal/" my/sync-root)
  "Directory holding personal org files.")

(defconst my/family-root (expand-file-name "family/" my/sync-root)
  "Directory holding family org files.")
...
(use-package org
  ...
  (org-agenda-files my/org-task-files)
  (org-todo-keywords
...
(defun my/org-open-weekly-review ()
  "Open weekly.org and refresh its dynamic block(s) (the clocktable)."
  (interactive)
  (let ((file (expand-file-name "weekly.org" my/org-root)))
    (with-current-buffer (find-file-noselect file)
      (let ((buffer-undo-list t))
        (org-update-all-dblocks))
      (pop-to-buffer (current-buffer)))))

(global-set-key (kbd "C-c R") #'my/org-open-weekly-review)
